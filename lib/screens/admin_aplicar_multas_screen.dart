...
class AdminAplicarMultasScreen extends StatefulWidget {
  @override
  _AdminAplicarMultasScreenState createState() => _AdminAplicarMultasScreenState();
}

class _AdminAplicarMultasScreenState extends State<AdminAplicarMultasScreen> {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredDetalles = _allDetalles.where((detalle) {
          ...
          return (detalle.multaDisplay.toLowerCase().contains(searchLower)) ||
                 (detalle.propiedadDisplay.toLowerCase().contains(searchLower));
        }).toList();
      }
    });
  }
  ...
  void _showAplicarMultaDialog() async {
    ...
    try {
      ...
      final futures = await Future.wait([
        AdminDetalleMultaService.getTiposMulta(),
        AdminDetalleMultaService.getPropiedades(),
      ]);
      
      tiposMulta = futures[0] as List<Multa>;
      propiedades = futures[1] as List<Propiedad>;
    } catch (e) {
  ...
  void _showDeleteDialog(DetalleMulta detalle) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '¿Estás seguro de que deseas eliminar la multa aplicada a ${detalle.propiedadDisplay}?',
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 8),
                                  Text(
                                    'Aplica la primera multa a una unidad',
                                    style: TextStyle(
                                      color: Colors.grey.shade600,
                                      fontSize: 14,
                                    ),
                                  ...
                                  ElevatedButton.icon(
                                    ...
                                    label: Text('Aplicar primera multa'),
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: Colors.red,
                                      foregroundColor: Colors.white,
                                      padding: EdgeInsets.symmetric(horizontal: 24, vertical: 12),
